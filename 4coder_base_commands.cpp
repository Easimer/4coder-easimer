    ...
    for (;;){
        ...
        if (!string_change){
            if (match_key_code(&in, KeyCode_PageDown) ||
                ...
                (match_key_code(&in, KeyCode_F) && has_modifier(&in.event.key.modifiers, KeyCode_Control))){
                change_scan = Scan_Forward;
                do_scan_action = true;
            }
            else if (match_key_code(&in, KeyCode_PageUp) ||
                     match_key_code(&in, KeyCode_Up) ||
                     (match_key_code(&in, KeyCode_R) && has_modifier(&in.event.key.modifiers, KeyCode_Control))){
